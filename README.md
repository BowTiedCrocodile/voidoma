...
Validated on Void **x86_64 glibc**, Hyprland **0.56.2** (Lua configuration), Rofi
...
fingerprint authentication block is preserved in `hyprlock.conf`. Symlink
...
numbered workspace. `python3 restore-scratchpad.py` moves all scratchpad windows
...
`touchpad.lua` defines this map. Pinch is not intercepted by the compositor.
...
writes. The first internal backlight is used; see `python3 brightness --help` for
...
Changes are temporary; edit `hyprland.lua` for persistent rules. Use
`python3 display-menu --list` for detection. Its output can contain monitor serials;
...
```sh
sudo sh setup-bluetooth.sh "$USER"
```
...
```sh
python3 setup-login.py "$USER" --dry-run
sudo python3 setup-login.py "$USER"
```
...
Edit `hyprland.lua`, `touchpad.lua`, `waybar.json`, `waybar.css`, `menu.rasi`, and
`mako.conf`. Application choices live in `hyprland.lua` and `void-menu`. The theme
uses DejaVu fonts. Wallpaper source and a rendered PNG are included.

Small `apply-*.py` migration helpers update selected settings for existing installs;
they are not required for a fresh install. Run them as the normal desktop user.
`apply-style.py` is an alias for installing the whole bundle with backups.
...
`check.sh` also requires the desktop dependencies and parses Hyprland/Rofi configs.
Neither replaces physical input, display, or authentication testing.

See [release review](RELEASE_REVIEW.md) for findings, scope, and known limitations.
